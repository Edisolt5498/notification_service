...
import faang.school.notificationservice.messaging.LikeEventMessageBuilder;
...
public class LikeEventListener implements MessageListener {
    ...
    private final LikeEventMessageBuilder likeEventMessageBuilder;
    ...
    public void onMessage(Message message, byte[] pattern) {
        ...
        try {
            ...
            var likeEvent = objectMapper.readValue(payload, LikeEvent.class);
            var messageToSend = likeEventMessageBuilder.buildMessage(likeEvent, Locale.getDefault());

            sendNotification(likeEvent, messageToSend);
    ...
    private void sendNotification(LikeEvent likeEvent, String messageToSend) {
        var userDto = getUser(likeEvent.postAuthorId());
